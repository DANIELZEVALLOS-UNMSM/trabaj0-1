clc;
clear;
close all;

% Datos
f = [100 120 145 170 200 235 270 310 355 405 460 520 585 655 ...
    730 810 895 985 1080 1180 1290 1410 1540 1680 1830 ...
    1990 2160 2340 2530 2730];

Z = [152.3 149.1 146.8 144.9 142.0 139.5 137.9 136.1 134.8 ...
    133.6 132.7 131.9 131.4 131.1 130.9 131.0 131.3 131.9 ...
    132.7 133.8 135.2 136.9 138.9 141.1 143.5 146.1 149.0 ...
    152.2 155.6 159.2];

% Malla fina
x = linspace(min(f),max(f),1000);

% Polinomio grado 10
p10 = polyfit(f,Z,10);
y10 = polyval(p10,x);

% Spline cubico natural
spline_Z = spline(f,Z,x);

% Grafica
figure

plot(f,Z,'ko','MarkerFaceColor','k')
hold on

plot(x,y10,'b','LineWidth',1.5)
plot(x,spline_Z,'r','LineWidth',1.5)

title('Spline Cubico vs Polinomio')
xlabel('Frecuencia (Hz)')
ylabel('|Z| (Ohm)')

legend('Datos','Polinomio grado 10','Spline cubico')
grid on

% Evaluacion en 1000 Hz
Z_pol = polyval(p10,1000);

Z_spline = spline(f,Z,1000);

fprintf('Polinomio en 1000 Hz = %.4f Ohm\n',Z_pol);

fprintf('Spline en 1000 Hz = %.4f Ohm\n',Z_spline);
